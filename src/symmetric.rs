...
impl SymmetricKey {
    ...
    pub fn encrypt(&mut self, message: Vec<u8>) -> Option<Vec<u8>> {
        ...
        match secretbox::xsalsa_easy(&mut encrypted, &nonce, &message, &self.key.lock()) {
            Err(_) => return None,
            Ok(_) => (),
        }
    ...
    pub fn decrypt(&mut self, ciphertext: Vec<u8>) -> Option<Vec<u8>> {
        ...
        secretbox::xsalsa_open_easy(&mut message, &ciphered, &nonce, &self.key.lock()).ok()?;
        return Some(message);
    }
}
